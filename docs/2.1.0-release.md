...
The desktop public SDK now has two supported entry points. Existing consumers keep the frozen V1-V12 compatibility header:
...
Consumers should request and null-check the lowest legacy interface version containing the methods they need. New integrations may instead discover independently versioned Core, Controller, and GameThread tables through `PrismaUI_F4_Modern_API.h`.

The current canonical public desktop SDK mirror is pinned to Git blob:

```text
23755e874f311a4294e662d3f6dc332547de5142
```

The current compatibility header is sourced from Prisma-Matrix commit `61848ed3fa65f41f4fb0f2d169f995a41951f62f`. The original 2.1.0 release source and the current API source are tracked separately by `prisma-mcp` so release provenance is not confused with later SDK/API evolution on the same release line.
...
- the canonical desktop V1-V12 compatibility SDK mirror;
- the modern Core/Controller/GameThread feature-table SDK mirror;
