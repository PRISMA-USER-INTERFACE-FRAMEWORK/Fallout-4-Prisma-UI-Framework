...
window, and report behaviour/animation defects that would otherwise only show up as an inâ€‘game
crash or a broken animation. This is meant for loadâ€‘order authors and for gating a build in CI.
...
`--scan-modlist` and `--scan-clips` read a **Mod Organizer 2 instance** â€” see
...
| Exit | Meaning |
|---|---|
| `0` | Clean â€” nothing inspected was defective. |
| `1` | Defects found â€” corrupt files, missing animations, or unresolved clips. |
| `2` | Usage or configuration error â€” a missing/omitted path argument, or an MO2 instance that could not be opened. |
...
- **Even a usage/config failure emits JSON** â€” a bad path or an unopenable instance produces a valid
...
`--scan-modlist` and `--scan-clips` take the **root of a Mod Organizer 2 instance** â€” the folder
...
- **`mods/`** â€” the mod library. Required.
...
- **A Fallout 4 `Data` folder.** Resolved from the `gamePath` in `ModOrganizer.ini`; if that is not
  available, a `Stock Folder/Data` beside the instance is used. This provides the baseâ€‘game files
  that mods override.
- **The overwrite folder** (optional) â€” taken from the INI's `overwrite_directory`, else
...
These scans validate **virtualâ€‘path resolution and the structure of the files they can describe**:
...
- **Existence is not loadability.** Whether a *resolved* animation's payload can actually be decoded
  and bound at runtime is a separate concern tracked in issue #201 (and its followâ€‘ups).
- **Undescribable layouts are skipped, not validated.** `--scan-archives` only structureâ€‘checks FO4
...
passed its structure check" â€” not "the load order cannot possibly fault."
